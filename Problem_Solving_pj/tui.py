...
def total_reviews(num_reviews):
    ...
    result = len(num_reviews)
    print(f"There are {result} reviews in the data set.")
...
def review_dates():
    ...
    """
    # TODO: Your code here
    review_date = input("Please enter a review dates in format mm/dd/yyyy split by space: ")
    total = review_date.split()
    print(total)
...
def display_reviews(reviews, cols=None):
    ...
    for i in reviews:
        if cols == None:
            new_data_list.append(reviews[i])
        else:
            for x in cols:
                new_list.append(reviews[x])
